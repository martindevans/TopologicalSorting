        ...
        internal static void CheckGraph(OrderedProcess<T> a, OrderedProcess<T> b) {
            if (a.Graph != b.Graph) {
                throw new ArgumentException(
                    $"process {a} is not associated with the same graph as process {b}");
            }
        }
                ...
                foreach (var process in processes) {
                    ...
                    var agreeableSets = result //from the set of result sets
                        .Where(set => set //select a candidate set to add to
                                      .Where(p => p.ResourcesSet.Overlaps(
                                                 process1
                                                     .Resources)) //select processes whose resource usage overlaps this one
                    ...
                    if (enumerable.IsEmpty()) {
                        ...
                        agreeableSet =
                            enumerable.Aggregate((a, b) => a.Count < b.Count ? a : b); //pick the smallest set
                    }
